...
export default defineEventHandler(async (event) => {
  ...
  const data = {
    auth: body.auth,
    errorCode: "ULO",
    rightName: "generalSettings_locations_editLocations",
    actionName: "edit-location",
  };
    ...
    .then(async (result) => {
      ...
      try {
        await prisma.Location.update({
          where: { id: body.currentLocation.id },
          data: {
            name: body.newLocation,
            accessibleBy: {
              set: body.users.map((user) => ({
                id: user.id,
              })),
            },
            visibleInFeed: {
              set: body.editedSelectedFeeds.map((f) => ({
                id: f.id,
              })),
            },
          },
        ...
        return {
          body: JSON.stringify({
            snackbar: {
              ...
              title: "Succes",
              text: "De wijzigingen zijn opgeslagen. De app wordt herstart. Een ogenblik aub.",
            },
          }),
        };
      } catch (error) {
